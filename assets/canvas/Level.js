...
var groupSalesVolume = 0, personalSalesVolume = 0,  progress, numberDistributors = 0, numberCustomers = 0,
		congrats, gameover, customerPosition = 1120, angle = {min: 0, max: 0}, color1=0xff0000, color2=0x80ff00,
		radialProgressBarArray = [], tweenArray = [], tween, baPosition = 10, customersArray = [], baArray = [], angleArray = [],
		customerDropArray = [], count = 0, addCustomer = Math.floor(Math.random() * 6000) + 1000, customerCount = 0,
    addBonusProduct = Math.floor(Math.random() * 600) + 100, productDropArray = [], productCount = 0;
/**
 * Level.
 */
function Level() {
...
Level.prototype.preload = function () {
  ...
  this.load.image('barContainer', 'assets/images/progressBarContainer.png');
...
Level.prototype.create = function () {
  ...
  _player.body.collideWorldBounds = true;
	...
	titleText = this.add.text(10, this.game.height-30, "Month", {fontSize: '16px', fill: '#000'});
  titleText.fixedToCamera = true;
  titleText.cameraOffset.setTo(10, this.game.height-30);

  progressBarContainer = this.add.image(0,0,"barContainer");
  progressBarContainer.height = 20;
  progressBarContainer.fixedToCamera = true;
  progressBarContainer.cameraOffset.setTo(70, this.game.height-30);
  ...
  progress.cameraOffset.setTo(70,this.game.height-30);
  ...
  gsvText = this.add.text(300, 0, 'Group Sales Volume: 0', {fontSize: '16px', fill: '#000'});
  gsvText.fixedToCamera = true;
  gsvText.cameraOffset.setTo(0,0);

  psvText = this.add.text(0, 0, 'Personal Sales Volume: 0', {fontSize: '16px', fill: '#000'});
  psvText.fixedToCamera = true;
  psvText.cameraOffset.setTo(300,0);

  retailText = this.add.text(600, 0, 'Retail Profit: 0', {fontSize: '16px', fill: '#000'});
  retailText.fixedToCamera = true;
  retailText.cameraOffset.setTo(600,0);

  var _customers = this.add.physicsGroup(Phaser.Physics.ARCADE);
	// public fields
...
Level.prototype.update = function() {
  ...
  this.game.physics.arcade.collide(this.fCustomers, this.fCollisionLayer);
  this.game.physics.arcade.collide(this.fFruits, this.fCollisionLayer);
  var playerOverlap = this.physics.arcade.overlap(this.fPlayer, this.fFruits, collectFruit, null, this);
  var customerOverlap = this.physics.arcade.overlap(this.fPlayer, this.fCustomers, this.collectCustomer, null, this);
  count++;
	if (!congrats.visible && !gameover.visible)
	{
  ...
  if (numberDistributors >= 1 && groupSalesVolume >= 2000)
	{
	...
	{
    radialProgressBarArray[i].clear();
    radialProgressBarArray[i].lineStyle(8, 0xff0000, 1);
    radialProgressBarArray[i].lineColor = Phaser.Color.interpolateColor(color1, color2, 360, angleArray[i].max, 1);
    radialProgressBarArray[i].arc(0,0,34, angleArray[i].min, this.game.math.degToRad(angleArray[i].max), false);
    radialProgressBarArray[i].endFill();
	}

	if (count%addCustomer == 0)
	{
		console.log("drop customer");
		var x = Math.floor(Math.random() * 1100) + 100;
		customerDropArray[customerCount] = this.fCustomers.create(x, 100, 'customer');
    customerDropArray[customerCount].body.gravity.y = Math.floor(Math.random() * 20) + 10;
    customerDropArray[customerCount].body.bounce.y = 0.6 + Math.random() * 0.2;
    customerCount++;
	}

	if (count%addBonusProduct == 0)
	{
		console.log("drop fruit");
    var x = Math.floor(Math.random() * 1100) + 100;
    var y = Math.floor(Math.random() * 300) + 10;
    //productDropArray[productCount] = this.fFruits.create(x, y, 'fruit');
    productDropArray[productCount] = this.add.sprite(x, y, 'objects', 'fruit', this.fFruits);
    productDropArray[productCount].body.gravity.y = Math.floor(Math.random() * 20) + 10;
    productDropArray[productCount].body.bounce.y = 0.6 + Math.random() * 0.2;
    productCount++;
	}
...
function collectFruit(player, fruit)
{
  ...
  groupSalesVolume += 10;
  personalSalesVolume += 10;
  gsvText.text = 'Group Sales Volume: ' + groupSalesVolume;
  psvText.text = 'Personal Sales Volume: ' + personalSalesVolume;
}

Level.prototype.collectCustomer = function(player, customer)
{
	var currentCustomer = numberCustomers;
  customer.kill();
  customerPosition = customerPosition - (currentCustomer* 75);

  customersArray[currentCustomer] = this.fCustomers.create(customerPosition, 0 , 'customer');
  customersArray[currentCustomer].fixedToCamera = true;
  customersArray[currentCustomer].cameraOffset.setTo(customerPosition,5);

  radialProgressBarArray[currentCustomer] = this.game.add.graphics(this.game.world.centerX, this.game.world.centerY);
  radialProgressBarArray[currentCustomer].lineStyle(8, 0xff0000, 1);
  radialProgressBarArray[currentCustomer].fixedToCamera = true;
  radialProgressBarArray[currentCustomer].cameraOffset.setTo(customerPosition+36,39);

  angleArray[currentCustomer] = {min: 0, max: 0}
	tweenArray[currentCustomer] = this.game.add.tween(angleArray[currentCustomer]).to( {max:360}, 20000, "Linear", true, 0, 0, false);
  tweenArray[currentCustomer].onComplete.add(function() {
    customersArray[currentCustomer].kill();
    radialProgressBarArray[currentCustomer].kill();

    baPosition = baPosition + (numberDistributors * 75);
    baArray[currentCustomer] = this.fCustomers.create(baPosition, 0, 'customer');
    baArray[currentCustomer].fixedToCamera = true;
    baArray[currentCustomer].cameraOffset.setTo(baPosition, 20);
    numberDistributors++;
    numberCustomers--;
    }, this);
